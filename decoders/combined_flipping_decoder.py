...
def confusion(predicted_positive: ArrayLike, actual_positive: NDArray[np.int_], negative: int) -> NDArray[np.int_]:
    ...
    :param predicted_positive:
    :param actual_positive:
    :param negative:
    :return:
    """
...
class CombinedUnifiedDecoder(Decoder):
    ...
    def __init__(self, ldpc_decoder: LogSpaDecoder, model_length: int,
                 ...
                 conf_center: int = 0, conf_slope: float = 0, debug: bool = False, bit_flip: float = 0, flip: bool = False
                 ) -> None:
    ...
    def mavlink_model_prediction(self, observation: NDArray[np.float_], cluster_id: int) -> \
        ...
        # find bits with valid model prediction of 0 and model std of zero. These are bits that are wrong for sure and should
        # be forced to the model's value.
        # If we learn the model online, need to be careful with bitwise_std, as it is not updated online. Thus require
        # valid_bits_p <= 0 to avoid forcing bits that are actually good.
        # If we don't learn the model online, bitwise_std==0 is enough to force required bits.
        if self.mavlink_learning:
            forced_bits: NDArray[np.int_] = np.where((valid_bits_p <= 0) & (bitwise_std == 0))[0]
        else:
            forced_bits: NDArray[np.int_] = np.where(bitwise_std == 0)[0]
