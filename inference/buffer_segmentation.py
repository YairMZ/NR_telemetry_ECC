...
class BufferStructure:
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        return self.structure == other if isinstance(other, dict) else False
...
class BufferSegmentation:
    ...
    def segment_buffer(self, buffer: bytes) -> tuple[npt.NDArray[np.object_], npt.NDArray[np.float_], dict[int, int]]:
        ...
        for byte_idx in range(len(buffer) - meta.protocol_overhead + 1):  # don't go over last bytes as to not raise a
            ...
            if is_valid_header(buffer[byte_idx:byte_idx + meta.header_len]):
                ...
                with contextlib.suppress(MAVError):
                    ...
                    msg_parts[byte_idx + meta.header_len:byte_idx + meta.header_len + header.length] = MsgParts.PAYLOAD
    ...
    def get_buffer_structure(self, buffer: bytes) -> BufferStructure:
        """The function returns a BufferStructure object which holds the structure of a buffer

        :param buffer: a buffer containing one or more MAVLink msgs
        :return: a BufferStructure object
        """
    ...
    def get_msg_fields(self, buffer: bytes) -> dict[str, Any]:
        ...
        :param buffer: a buffer containing one or more MAVLink msgs
        :return: a dictionary which holds the fields of a message in a buffer
        """
        msg_parts, bit_validity, buffer_structure = self.segment_buffer(buffer)
        if buffer_structure == {}:
            return {}
        fields = {}
        buffer_description = []
