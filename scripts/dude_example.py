from inference import OnlineDude
import numpy as np
...
k = int(0.5*np.log2(n))
dude = OnlineDude(channel_transition_matrix, loss_matrix, k, hard_decision=True)
# generate a random sequence of 0s and 1s (uniformly distributed)
sequence = np.random.randint(0, 2, n)
# set the initial context
dude.initial_context(tuple(sequence[:k]))
# denoise a sample
estimate, cost = dude.denoise_sample(sequence[k])
...
dude = OnlineDude(channel_transition_matrix2, loss_matrix, k, hard_decision=True)
p = 0.9
...
k = 4
dude = OnlineDude(channel_transition_matrix, loss_matrix, k, hard_decision=False)
...
print(f"Noise errors: {np.sum(noisy_sequence != sequence)}")
soft_estimates, costs = dude.denoise_sequence(noisy_sequence)
