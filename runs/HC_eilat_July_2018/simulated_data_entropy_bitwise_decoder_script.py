...
for p in bit_flip_p:
    ...
    for tx_idx in range(n):
        ...
        if decoded_flipping[-1][2] is False:
            print("entropy flipping ldpc, errors after decode: ", decoded_flipping[-1][5])
        print("tx id: ", tx_idx)
    ...
    results[-1]["decoded_flipping"] = decoded_flipping
    results[-1]["flipping_buffer_success_rate"] = sum(int(res[5] == 0) for res in decoded_flipping) / float(n)
    results[-1]["flipping_decoder_ber"] = sum(
