...
parser.add_argument("--maxflip", default=75*1e-3, help="maximal bit flip probability to consider", type=float)
...
with open('analysis/data_model.pickle', 'rb') as f:
    dist_model: NDArray[np.float_] = pickle.load(f).get('dist')
model_length = dist_model.shape[0]
...
cmd = f'python {__file__} --minflip {args.minflip} --maxflip {args.maxflip} --nflips {args.nflips} ' +\
      f" --ldpciterations {ldpc_iterations} --ent_threshold {thr} --clipping_factor {clipping_factor}"
...
def simulation_step(p: float) -> dict[str, Any]:
    ...
    print("successful entropy flipping decoding for bit flip p=", p, ", is: ",
          sum(int(res[-1] == 0) for res in decoded_flipping), "/", n)
    ...
    step_results["flipping_buffer_success_rate"] = sum(int(res[-1] == 0) for res in decoded_flipping) / float(n)
