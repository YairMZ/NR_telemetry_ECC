...
def simulation_step(p: float) -> dict[str, Any]:
    ...
    step_results["entropy_wbf_decoder_ber"] = sum(res[-1] for res in decoded_entropy_mwbf) / float(n * len(encoded[0]))
    step_results["entropy_mwbf_success_rate"] = sum(int(res[-1] == 0) for res in decoded_entropy_wbf) / float(n)
