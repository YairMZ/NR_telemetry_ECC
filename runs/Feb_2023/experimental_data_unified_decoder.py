...
from decoders import CombinedUnifiedDecoder
...
parser = argparse.ArgumentParser(description='Run decoding on experimental data using unified decoder.')
...
data_model = None if bool(args.learn) else BufferModel.load('data/model_2023.json')
...
logger.info(f"separate_models: {args.separate_models}")

cmd = f'python {__file__} --ldpciterations {ldpc_iterations}  --learn {args.learn} --valid_factor {args.valid_factor} ' \
      f'--invalid_factor {args.invalid_factor} --valid_threshold {args.valid_threshold} ' \
      f'--invalid_threshold {args.invalid_threshold} --conf_center {args.conf_center} --conf_slope {args.conf_slope} ' \
      f'--dec_type {args.dec_type} --experiment_date {args.experiment_date} --ent_threshold {thr} ' \
      f'--clipping_factor {clipping_factor} --flip {args.flip} --raw_ber {args.raw_ber} --msg_type {args.msg_type} ' \
      f'--separate_models {args.separate_models}'
...
try:
    ...
    nr_decoder = CombinedUnifiedDecoder(LogSpaDecoder(h=h.to_sparse(), max_iter=args.ldpciterations,
                                        ...
                                        classifier_training_size=0, cluster=False,
                                        window_length=window_len, data_model=data_model, conf_center=args.conf_center,
                                        conf_slope=args.conf_slope, debug=True, bit_flip=args.raw_ber, flip=bool(args.flip))
    nr_decoder.set_buffer_structures([buffer_structure])
    decoded_ldpc = []
    decoded_nr = []
    ...
    for tx_idx in range(number_of_messages):
        ...
        if tx[tx_idx, 0] < 2:  # valid tx identified
            ...
            errors_idx = np.where(errors[tx_idx, :] == 1)[0]
        else:
            hamm = -1
            errors_idx = None
        decoded_ldpc.append((*d, hamm))
        d = nr_decoder.decode_buffer(rx[tx_idx, :], errors_idx)
        nr_success = d[2]
        hamm = hamming_distance(d[0], tx[tx_idx, :]) if tx[tx_idx, 0] < 2 else -1
        decoded_nr.append((*d, hamm))
        logger.info(f"tx id:, {tx_idx}, ldpc: {ldpc_success}, nr: {nr_success}")

    logger.info(f"successful pure decoding is: {sum(res[2] for res in decoded_ldpc)}/{number_of_messages}")
    logger.info(f"successful nr decoding is: {sum(res[2] for res in decoded_nr)}/{number_of_messages}")
    ...
    results["nr_buffer_success_rate"] = sum(res[2] for res in decoded_nr) / float(number_of_messages)
    results["ldpc_decoder_ber"] = sum(res[-1] for res in decoded_ldpc if res[-1] >= 0) / float(n * sum(good_tx))
    results["nr_decoder_ber"] = sum(res[-1] for res in decoded_nr if res[-1] >= 0) / float(n * sum(good_tx))
    results["input_ber"] = input_ber

    # save results
    with lzma.open(
            os.path.join(path, f'{timestamp}_experimental_data_analysis_unified_nr.xz'), "wb") as f:
        pickle.dump(results, f)

    results['data'] = results['data'].to_dict("list")
    results['decoded_nr'] = results['decoded_nr'].to_dict("list")
    results['decoded_ldpc'] = results['decoded_ldpc'].to_dict("list")
    results['args'] = args

    savemat(os.path.join(path, f'{timestamp}_experimental_data_analysis_unified_nr.mat'), results, do_compression=True)

    summary_txt = f'successful pure decoding is: {sum(res[2] for res in decoded_ldpc)}/{number_of_messages}\n' \
                  f'successful nr decoding is: {sum(res[2] for res in decoded_nr)}/{number_of_messages}'
